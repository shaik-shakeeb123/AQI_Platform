...
async def _get_with_retry(url: str, params: dict, *, error_code: str, label: str) -> dict:
    ...
    for attempt in range(1, _MAX_RETRIES + 1):
        try:
            ...
            last_exc = e
            last_status = None  # Clear status on network error
            wait = min(
                _RETRY_BASE_DELAY * (2 ** (attempt - 1)) + random.uniform(0.0, 0.5),
                _RETRY_MAX_DELAY,
            )
    ...
    if last_status == 429:
        raise ExternalAPIException(
            detail=f"{label}: HTTP 429 Too Many Requests. The upstream weather provider is currently rate limiting requests.",
